...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 08:54:43 2022
...
single_list = glob('/Users/ixl/Desktop/Cones/Single/*')
many_list = glob('/Users/ixl/Desktop/Cones/Many/*')
...
for sin in single_list:
    if 'xy' in sin:
        singles.append(np.loadtxt(sin,delimiter=' '))
...
for z in range(len(singles)):
    ...
    y_ind += length
    #plt.imshow(data)
    plt.plot(np.sum(data,axis=0)/max(np.sum(data,axis=0)),label=labels[z])
    #plt.plot(np.sum(data,axis=1),label='y sum')
    #plt.plot(np.sum(bins,axis=0),label=labels[z])
